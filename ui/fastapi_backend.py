...
async def android_chrome_192():
    ...
    icon_path = static_dir / "android-chrome-192x192.png"
    if icon_path.exists():
        return FileResponse(str(icon_path), media_type="image/png")
    raise HTTPException(status_code=404)
...
async def android_chrome_512():
    ...
    icon_path = static_dir / "android-chrome-512x512.png"
    if icon_path.exists():
        return FileResponse(str(icon_path), media_type="image/png")
    raise HTTPException(status_code=404)
...
def generate_checklist_pdf(checklist_items: List[Dict[str, Any]], situation: str, programs: List[str]) -> bytes:
    ...
    # Add footer
    footer_text = "Generated by BenefitsFlow - California Benefits Navigator<br/>For questions or assistance, visit: https://benefitsflow.org"
...
def checklist_to_markdown(checklist_items: List[Dict[str, Any]], situation: str, programs: List[str]) -> str:
    ...
    lines.append("*For questions or assistance, visit: https://benefitsflow.org*")
